...
static const char *TAG = "I2SClocklessVirtualLedDriveresp32s3";
#endif
...
static IRAM_ATTR bool dma_callback(gdma_channel_handle_t dma_chan,gdma_event_data_t *event_data, void *user_data);
static void IRAM_ATTR loadAndTranspose(I2SClocklessVirtualLedDriveresp32s3 *driver);
static void  transposeAll(I2SClocklessVirtualLedDriveresp32s3 *driver);
...
I2SClocklessVirtualLedDriveresp32s3DMABuffer * DMABuffersTampon[__NB_DMA_BUFFER+1];
class I2SClocklessVirtualLedDriveresp32s3
{
...
public:
    ...
    volatile xSemaphoreHandle I2SClocklessVirtualLedDriveresp32s3_sem = NULL;
    volatile xSemaphoreHandle I2SClocklessVirtualLedDriveresp32s3_semSync = NULL;
    volatile xSemaphoreHandle I2SClocklessVirtualLedDriveresp32s3_semDisp = NULL;
    volatile xSemaphoreHandle I2SClocklessVirtualLedDriveresp32s3_waitDisp = NULL;
    bool isOffsetDisplay;
    ...
    I2SClocklessVirtualLedDriveresp32s3() {};
    ...
    LCD_CAM.lcd_clock.lcd_clkm_div_b = CLOCK_DIV_A;
...
*/
    ...
    LCD_CAM.lcd_user.lcd_start=0;
        ...
        if (I2SClocklessVirtualLedDriveresp32s3_semSync == NULL)
        {
            I2SClocklessVirtualLedDriveresp32s3_semSync = xSemaphoreCreateBinary();
        }
        if (I2SClocklessVirtualLedDriveresp32s3_semDisp == NULL)
        {
            I2SClocklessVirtualLedDriveresp32s3_semDisp = xSemaphoreCreateBinary();
        }
    }
...
{
                 DMABuffersTampon[num_buff]->dw0.suc_eof = 0;
}
    if(num_buff>0)
 {
...
DMABuffersTampon[__NB_DMA_BUFFER]->next = DMABuffersTampon[1];
    ...
    void waitDisplay()
    {
        ...
        if (isDisplaying == true)
        {
            ...
            xSemaphoreTake(I2SClocklessVirtualLedDriveresp32s3_waitDisp, xDelay);
        }
    ...
    void _runShowPixelsOnCore()
    {
        ...
        if (I2SClocklessVirtualLedDriveresp32s3_returnTaskHandle == 0)
        {

            I2SClocklessVirtualLedDriveresp32s3_returnTaskHandle = xTaskGetCurrentTaskHandle();

            // -- Trigger the show task
            xTaskNotifyGive(I2SClocklessVirtualLedDriveresp32s3_dispTaskHandle);

            // -- Wait to be notified that it's done
            // ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
            // delay(100);
            // interrupts();
            I2SClocklessVirtualLedDriveresp32s3_returnTaskHandle = 0;
        }
    }

    void disableShowPixelsOnCore()
    {
        if (I2SClocklessVirtualLedDriveresp32s3_dispTaskHandle)
        {
            vTaskDelete(I2SClocklessVirtualLedDriveresp32s3_dispTaskHandle);
        }
    ...
    void enableShowPixelsOnCore(int corenum)
    {
        ...
        runCore = corenum;
        isRunOnCore = true;
        xTaskCreatePinnedToCore(showPixelsTask, "showPixelsTask", 2000, this, 3, &I2SClocklessVirtualLedDriveresp32s3_dispTaskHandle, corenum);
    }
...
*/
      ...
      // _defaulthmap = (uint16_t *)malloc(NUM_LEDS_PER_STRIP * NBIS2SERIALPINS * 8 * 2 + 2);
        ...
        if (I2SClocklessVirtualLedDriveresp32s3_waitDisp == NULL)
        {
            I2SClocklessVirtualLedDriveresp32s3_waitDisp = xSemaphoreCreateCounting(10, 0);
        }
    ...
    I2SClocklessVirtualLedDriveresp32s3DMABuffer **DMABuffersTransposed = NULL;
    // buffer array for the regular way
    //I2SClocklessVirtualLedDriveresp32s3DMABuffer *DMABuffersTampon[__NB_DMA_BUFFER + 2];

    I2SClocklessVirtualLedDriveresp32s3DMABuffer *allocateDMABuffer()
    {
        I2SClocklessVirtualLedDriveresp32s3DMABuffer *b = (I2SClocklessVirtualLedDriveresp32s3DMABuffer *)heap_caps_malloc(sizeof(I2SClocklessVirtualLedDriveresp32s3DMABuffer), MALLOC_CAP_DMA);
        if (!b)
        {
            ESP_LOGE(TAG, "No more memory\n");
            return NULL;
 ...
 b->dw0.owner = DMA_DESCRIPTOR_BUFFER_OWNER_DMA;
        ...
        memset(b->buffer, 0, WS2812_DMA_DESCRIPTOR_BUFFER_MAX_SIZE);
 b->dw0.size = WS2812_DMA_DESCRIPTOR_BUFFER_MAX_SIZE;
                b->dw0.length = WS2812_DMA_DESCRIPTOR_BUFFER_MAX_SIZE;
    ...
    */
    ...
    void i2sStart(I2SClocklessVirtualLedDriveresp32s3DMABuffer *startBuffer)
    {
/*
        i2sReset();
        framesync = false;
...
/*
    ...
    // return;
    I2SClocklessVirtualLedDriveresp32s3 *cont = (I2SClocklessVirtualLedDriveresp32s3 *)arg;
    ...
    if (GET_PERI_REG_BITS(I2S_INT_ST_REG(I2S_DEVICE), I2S_OUT_EOF_INT_ST_S, I2S_OUT_EOF_INT_ST_S))
    {
        ...
        if (((I2SClocklessVirtualLedDriveresp32s3 *)arg)->transpose)
        {
        ...
        else
        {
            if (cont->framesync)
            {
                ...
                xSemaphoreGiveFromISR(cont->I2SClocklessVirtualLedDriveresp32s3_semSync, &HPTaskAwoken);
...
#if (I2S_MAPPING_MODE & I2S_MAPPING_MODE_OPTION_DIRECT_CALCULATION) == 0
static inline __attribute__((always_inline)) void IRAM_ATTR loadAndTranspose(I2SClocklessVirtualLedDriveresp32s3 *driver)
{
...
static void showPixelsTask(void *pvParameters)
{
    I2SClocklessVirtualLedDriveresp32s3 *cont = (I2SClocklessVirtualLedDriveresp32s3 *)pvParameters;
    ...
    esp_err_t e = esp_intr_alloc(interruptSource, ESP_INTR_FLAG_INTRDISABLED | ESP_INTR_FLAG_LEVEL3 | ESP_INTR_FLAG_IRAM, &_I2SClocklessVirtualLedDriveresp32s3interruptHandler, cont, &cont->_gI2SClocklessDriver_intr_handle);
    if (e != ESP_OK)
    {
